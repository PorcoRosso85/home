 ...
 * ```typescript
 * // 関数の戻り値として共用型を定義する例
 * function processData(input: string): string | Error {
 *   try {
 *     // 処理ロジック
 *     return "処理結果";
 *   } catch (err) {
 *     return new Error("処理エラー"); 
 *   }
 * }
 * ```
 * 
 ...
 * {
 *   "type": "union",
 *   "description": "データ処理の結果または処理時のエラー",
 *   "variants": [
 *     {
 *       "type": "string",
 *       "description": "処理成功時の結果文字列"
 *     },
 *     {
 *       "type": "object",
 *       "description": "処理失敗時のエラーオブジェクト",
 *       "$ref": "Error.json"
 *     }
 *   ]
 * }
...
export interface TypeDefinition {
  ...
  varargs?: boolean;
  defaultsStrategy?: 'eager' | 'lazy' | 'none';
  // 共用型（Union型）のサポート
