...
from upsert.application.init_operations import (
...
def get_error_help(error_type: str) -> str:
    ...
    error_help = {
        "DB_PATH_ERROR": "データベースパスの指定に問題があります。有効なパスを指定してください。\n"
                        "例: --fp-init または --fp-init --register-data",
...
def get_command_examples() -> List[str]:
    ...
    """
    return [
        "LD_LIBRARY_PATH=\"/nix/store/p44qan69linp3ii0xrviypsw2j4qdcp2-gcc-13.2.0-lib/lib/\":$LD_LIBRARY_PATH /home/nixos/bin/src/kuzu/upsert/.venv/bin/python /home/nixos/bin/src/kuzu/upsert/__main__.py --fp-init --register-data",
        "LD_LIBRARY_PATH=\"/nix/store/p44qan69linp3ii0xrviypsw2j4qdcp2-gcc-13.2.0-lib/lib/\":$LD_LIBRARY_PATH /home/nixos/bin/src/kuzu/upsert/.venv/bin/python /home/nixos/bin/src/kuzu/upsert/__main__.py --fp-init --register-data --data-dir=/path/to/data"
    ]
...
def handle_fp_init(db_path: Optional[str] = None, in_memory: bool = False, 
    ...
    # デバッグログ
    log_debug(f"handle_fp_init 関数パラメータ: db_path={db_path}, in_memory={in_memory}, register_data={register_data}, data_dir={data_dir}, recursive={recursive}")
            ...
            if not init_files:
                ...
                for file_path in init_files:
                    ...
                    if fp_is_error(file_result):
                        failed_count += 1
                        log_warning(f"ファイル処理失敗: {file_path} - {file_result['message']}")
                    else:
                        processed_count += 1
                        nodes_count += file_result["data"]["nodes_count"]
                        edges_count += file_result["data"]["edges_count"]
                        
                        # スキップされたノードとエッジの数を収集
                        skipped_nodes += file_result["data"]["nodes_skipped"]
                        skipped_edges += file_result["data"]["edges_skipped"]
