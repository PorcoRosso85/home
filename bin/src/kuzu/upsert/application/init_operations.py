...
def insert_edges(
    ...
    if is_error(tables_result):
        ...
        if not table_details.get("InitEdge", False):
            log_warning("エッジテーブルが見つかりません。エッジ挿入をスキップします。")
            return create_success(
...
def save_init_data(
    ...
    # 結果に追加情報を付加
    if result["success"]:
        result["data"]["tables_status"] = (
            tables_check["data"]["tables"] if tables_check["success"] else 
            tables_check["details"]["tables"]
        )
...
def get_root_nodes(
    ...
    if is_error(tables_result):
        ...
        if not table_details.get("InitNode", False) or not table_details.get("InitEdge", False):
            log_debug(f"ルートノード取得前のテーブル存在確認: InitNode={table_details.get('InitNode', False)}, InitEdge={table_details.get('InitEdge', False)}")
