...
export function useDmlQueries() {
   ...
   */
  const insertSampleLocationURI = useCallback(async (conn: any): Promise<void> => {
    logger.debug('サンプルLocationURIノードを作成中...');
    
    const locationURIs: LocationURIData[] = [
      // ファイルシステムURI
    ...
    try {
      ...
      logger.debug('サンプルLocationURIデータの挿入完了');
    } catch (error) {
