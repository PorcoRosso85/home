  ...
  describe("1テーブル多粒度モデルテスト", () => {
    ...
    it("異常系: 不正なパラメータ処理", async () => {
      ...
      try {
        ...
        const reconstructQuery = await getQuery("reconstruct_path_from_node");
        const emptyNodeId = "";
        const reconstructParameterized = buildParameterizedQuery(reconstructQuery.data!, { nodeId: emptyNodeId });
        const reconstructResult = await conn.query(reconstructParameterized);
        const reconstructRows = await reconstructResult.getAll();
        
        assertEquals(reconstructRows.length, 0, "空文字ノードIDの場合は結果が0件であるべき");
        
        // null相当のパラメータテスト
        const nullParameterized = buildParameterizedQuery(reconstructQuery.data!, { nodeId: 'NULL' });
        const nullResult = await conn.query(nullParameterized);
        const nullRows = await nullResult.getAll();
        
        assertEquals(nullRows.length, 0, "NULL値の場合は結果が0件であるべき");
        
        console.log("不正パラメータ異常系テスト完了");
        
        await conn.close();
        await db.close();
      } catch (e) {
        console.error('不正パラメータ異常系テストエラー:', e);
        throw e;
      }
    });
