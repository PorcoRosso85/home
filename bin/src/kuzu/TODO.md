...
### 0. YAMLからCypher生成処理のインフラ層への移行
- `process_init_file`関数とその関連機能をアプリケーション層からインフラ層へ移動
- YAMLファイル（CONVENTION.yaml含む）の読み込み処理を責務に応じて適切に配置
- ノード/エッジへの変換とCypherクエリテンプレート適用の責務を明確に分離
- 後方互換性は考慮せず、CONVENTION.yamlの規約に準拠した純粋関数のみを使用
- 既存の関数は削除するか、新しい実装へのシンプルな橋渡しとして再実装する
