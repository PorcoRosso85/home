...
class TestFTSApplication:
    ...
    def test_indexing_documents_with_distinct_ids_stores_separately(self, fts_service):
        ...
        # 複数のドキュメントをインデックス
        documents = [
            {"id": "1", "title": "最初のドキュメント", "content": "これは最初のドキュメントの内容です"},
            {"id": "2", "title": "2番目のドキュメント", "content": "これは2番目のドキュメントの内容です"}
        ]
    ...
    def test_indexed_documents_are_searchable_immediately(self, fts_service):
        ...
        # ドキュメントをインデックス
        documents = [
            {"id": "TEST001", "title": "テストドキュメント", "content": "これはテスト用のドキュメントです"}
        ]
...
class TestFTSFeatures:
    ...
    def fts_service_with_data(self):
        ...
        documents = [
            {"id": "1", "title": "Python Programming Guide", "content": "Learn Python programming basics"},
            {"id": "2", "title": "JavaScript Tutorial", "content": "Master JavaScript programming"},
            {"id": "3", "title": "Database Design", "content": "Design efficient databases with SQL"},
    ...
    def test_keyword_search_returns_matching_documents(self, fts_service_with_data):
        ...
        # Debug output
        if not result["ok"] or len(result["results"]) == 0:
