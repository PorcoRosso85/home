...
def install_fts_extension(connection: Any) -> Tuple[bool, Optional[Dict[str, Any]]]:
    """
...
def check_fts_extension(connection: Any) -> Tuple[bool, Optional[Dict[str, Any]]]:
    """
    FTS拡張が利用可能かチェックする
    ...
    try:
        ...
        try:
            connection.execute(f"CREATE NODE TABLE {temp_table} (content STRING, PRIMARY KEY (content));")
        except:
        ...
        if any(pattern in error_msg.lower() for pattern in ["does not exist", "unknown function", "create_fts_index"]):
            return False, {
...
def initialize_fts_schema(connection: Any) -> Tuple[bool, Optional[Dict[str, Any]]]:
    """
    FTS用のスキーマを初期化する
...
def create_fts_index(connection: Any, config: Dict[str, Any]) -> Tuple[bool, Optional[Dict[str, Any]]]:
    """
    FTSインデックスを作成する
...
def drop_fts_index(connection: Any, index_name: str) -> Tuple[bool, Optional[Dict[str, Any]]]:
    """
    FTSインデックスを削除する
...
def list_fts_indexes(connection: Any) -> Tuple[bool, List[Dict[str, Any]], Optional[Dict[str, Any]]]:
    """
    FTSインデックスの一覧を取得する
    ...
    try:
        # Query FTS indexes
        # KuzuDB FTS extension doesn't have a standard list function
        # This is a mock implementation that would need to be replaced
        # with actual index tracking in a production system
        indexes = []
        
        # In a real implementation, we would query metadata tables or
        # maintain our own index registry. For now, return empty list.
...
def get_fts_index_info(connection: Any, index_name: str) -> Tuple[bool, Optional[Dict[str, Any]], Optional[Dict[str, Any]]]:
    """
    FTSインデックスの情報を取得する
    ...
    try:
        # Get FTS index info
        # This is a simplified implementation - actual implementation may differ
...
def query_fts_index(connection: Any, index_name: str, query: str, limit: int = 10) -> Tuple[bool, List[Dict[str, Any]], Optional[Dict[str, Any]]]:
    """
