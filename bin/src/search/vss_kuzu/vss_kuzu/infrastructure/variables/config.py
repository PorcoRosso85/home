...
def merge_config(base_config: VSSConfig, **overrides) -> VSSConfig:
    ...
    new_sources = base_config._sources.copy() if base_config._sources else {}
    for key in overrides:
        if key in new_sources:
            new_sources[key] = 'override'
    
    # dataclassのreplace機能を使用して新しいインスタンスを作成
    return replace(base_config, _sources=new_sources, **overrides)
