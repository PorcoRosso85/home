#!/usr/bin/env python3
"""
VSS (Vector Similarity Search) Service - Function-based Architecture
"""
...
class VSSContext(TypedDict):
    """VSS context containing database state and configuration"""
    db_path: str
    in_memory: bool
    dimension: int
    db: Optional[Any]
    ...
    vector_extension_available: Optional[bool]
    subprocess_wrapper: Optional[Any]


def create_vss_context(db_path: str = "./kuzu_db", in_memory: bool = False) -> VSSContext:
    """Create a new VSS context"""
    context: VSSContext = {
        "db_path": db_path,
        "in_memory": in_memory,
        ...
        "vector_extension_available": None,
    ...
    if 'pytest' in sys.modules:
        try:
            ...
            context["subprocess_wrapper"] = VectorSubprocessWrapper(
...
def get_connection(context: VSSContext) -> Any:
    ...
    if context["conn"] is None:
        ...
        db_path = IN_MEMORY_DB_PATH if context["in_memory"] else context["db_path"]
        ...
        context["db"] = db_result
        
        conn_result = create_connection(context["db"])
        # Check if it's an error (VectorSearchError has 'ok' attribute set to False)
...
def initialize_schema(context: VSSContext) -> None:
    """Initialize database schema and check VECTOR extension availability"""
    conn = context["conn"]
    
    # Check VECTOR extension availability
    context["vector_extension_available"] = check_vector_extension(conn)
    ...
    try:
        conn.execute(f"""
            CREATE NODE TABLE IF NOT EXISTS {DOCUMENT_TABLE_NAME} (
                id STRING,
                content STRING,
                embedding FLOAT[{context["dimension"]}],
                PRIMARY KEY (id)
            )
        """)
...
def check_vector_extension(conn: Any) -> bool:
...
def get_embedding_service(context: VSSContext) -> Any:
    ...
    if context["embedding_service"] is None:
        ...
        context["embedding_service"] = StandaloneEmbeddingService()
        context["dimension"] = context["embedding_service"].dimension
    
    return context["embedding_service"]


def ensure_vector_index(conn: Any) -> None:
...
def search(context: VSSContext, input_data: Dict[str, Any]) -> Union[VectorSearchResult, VectorSearchError]:
    """
    ...
    try:
        ...
        # Check VECTOR extension availability
        if not context["vector_extension_available"]:
        ...
        embedding_service = get_embedding_service(context)
        
        # Get or compute query embedding
        if query_vector:
            # Use provided vector
            query_embedding = query_vector
            if len(query_embedding) != context["dimension"]:
                return VectorSearchError(
                    ok=False,
...
def index_documents(context: VSSContext, documents: List[Dict[str, str]]) -> Union[VectorIndexResult, VectorSearchError]:
    """
...
# Legacy compatibility: Create VSSService class that wraps the functions
class VSSService:
    """Vector Similarity Search service - Legacy wrapper for function-based architecture"""
    
    def __init__(self, db_path: str = "./kuzu_db", in_memory: bool = False):
        self.context = create_vss_context(db_path, in_memory)
        # Keep these for backward compatibility
        self.db_path = db_path
        self.in_memory = in_memory
        self.dimension = self.context["dimension"]
        self._db = None
        self._conn = None
        self._embedding_service = None
        self._vector_extension_available = None
        self._subprocess_wrapper = self.context["subprocess_wrapper"]
    
    def _get_connection(self):
        """Get or create database connection"""
        conn = get_connection(self.context)
        # Update instance variables for backward compatibility
        self._db = self.context["db"]
        self._conn = self.context["conn"]
        self._vector_extension_available = self.context["vector_extension_available"]
        return conn
    
    def _get_embedding_service(self):
        """Get or create embedding service"""
        service = get_embedding_service(self.context)
