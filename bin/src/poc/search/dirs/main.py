...
def _scan_directory(
    root_path: str, skip_hidden: bool, skip_empty: bool, max_depth: Optional[int], current_depth: int
) -> List[DirectoryInfo]:
    ...
    try:
        for item in os.listdir(root_path):
            ...
            if os.path.isdir(full_path):
                ...
                # Recurse
                results.extend(_scan_directory(full_path, skip_hidden, skip_empty, max_depth, current_depth + 1))
...
def create_directory_scanner(root_path: str, db_path: str) -> Dict[str, Callable]:
    ...
    conn_result = create_database_and_connection(db_path if db_path != ":memory:" else None, db_path == ":memory:")
    ...
    if not conn_result["ok"]:
        ...
        return {
            ...
            "get_status": lambda: DBStatus(total_directories=0, indexed_directories=0, last_scan=None, db_size_mb=0.0),
    ...
    def full_scan(skip_empty: bool, skip_hidden: bool, generate_embeddings: bool) -> ScanResult:
        ...
        try:
            ...
            for dir_info in current_dirs:
                if dir_info["path"] in db_paths:
                    ...
                    result = conn.execute(
                        "MATCH (d:Directory {path: $path}) RETURN d.has_readme", {"path": dir_info["path"]}
                    )
                    if result.has_next():
                        row = result.get_next()
            ...
            return DiffSuccess(ok=True, added=added, modified=modified, deleted=deleted)
    ...
    def incremental_update() -> ScanResult:
        ...
        try:
            ...
            for path in diff_result["modified"]:
                ...
                """,
                    {"path": path, "has_readme": has_readme},
                )
    ...
    def search_hybrid(query: str, alpha: float) -> SearchResult:
        ...
        try:
            ...
            for hit in fts_result["hits"]:
                path_scores[hit["path"]] = {"fts_score": hit["score"], "vss_score": 0.0, "hit": hit}
            ...
            for hit in vss_result["hits"]:
                if hit["path"] in path_scores:
                    ...
                    path_scores[hit["path"]] = {"fts_score": 0.0, "vss_score": hit["score"], "hit": hit}

            # Calculate hybrid scores
            hits = []
            for path, scores in path_scores.items():
                hybrid_score = alpha * scores["fts_score"] + (1 - alpha) * scores["vss_score"]
                hit = scores["hit"]
    ...
    def extract_metadata(path: str) -> MetadataResult:
        ...
        try:
            ...
            if readme := _get_readme_content(path):
                ...
                return MetadataSuccess(ok=True, description=desc[:200], source="readme")

            # Check flake.nix
            if desc := _extract_flake_description(path):
                return MetadataSuccess(ok=True, description=desc, source="flake")

            # Check package.json
            if desc := _extract_package_json_description(path):
                return MetadataSuccess(ok=True, description=desc, source="package.json")

            # Check Python docstring
            if desc := _extract_python_docstring(path):
                return MetadataSuccess(ok=True, description=desc, source="docstring")

            # No metadata found
            return MetadataSuccess(ok=True, description=None, source="none")
    ...
    def get_status() -> DBStatus:
        ...
        try:
            ...
            result = conn.execute("MATCH (d:Directory) WHERE d.embedding IS NOT NULL RETURN COUNT(*)")
            ...
            return DBStatus(total_directories=0, indexed_directories=0, last_scan=None, db_size_mb=0.0)
