...
def test_is_empty_dir_ファイルあり_False返却(temp_dir):
    """ファイルがあるディレクトリは空ではない"""
...
def test_get_file_count_正確なカウント_ファイル数返却(temp_dir):
...
def test_get_subdirs_サブディレクトリ名_リスト返却(temp_dir):
    """直下のサブディレクトリ名のリストを返却"""
    test_dir = f"{temp_dir}/test"
    os.makedirs(test_dir)
...
def test_extract_flake_description_正常_説明文返却(temp_dir):
...
def test_extract_flake_description_ファイルなし_None返却(temp_dir):
    """flake.nixがない場合はNone"""
    result = _extract_flake_description(temp_dir)
    assert result is None


def test_extract_package_json_description_正常_説明文返却(temp_dir):
...
def test_extract_python_docstring_正常_docstring返却(temp_dir):
...
def test_get_readme_content_存在する場合_内容返却(temp_dir):
    """READMEファイルの内容を取得"""
    content = "# Test README\n\nThis is a test."
...
def test_get_readme_content_大文字小文字_対応(temp_dir):
