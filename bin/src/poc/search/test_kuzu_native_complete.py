...
def run_native_test():
    ...
    hybrid_found = vss_found.intersection(
        {id for id in vss_found if any(keyword in id.lower() for keyword in ["sec", "auth", "privacy", "audit"])}
    )
