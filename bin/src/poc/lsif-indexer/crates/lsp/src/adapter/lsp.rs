...
pub trait LspAdapter {
    ...
    fn get_init_params(&self) -> InitializeParams {
        let uri = Url::from_directory_path(std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."))).unwrap();
        #[allow(deprecated)]
        InitializeParams {
...
impl GenericLspClient {
    ...
    fn initialize_with_params(
        ...
        // 2-stage timeout configuration: 30 seconds for initialize phase
        let timeout = timeout.unwrap_or_else(|| {
            match self.language_id.as_str() {
                "nix" => Duration::from_secs(30), // 30 seconds for nixd initialization
                _ => self.health_checker
    ...
    pub fn initialize(&mut self, project_root: &Path, timeout: Option<Duration>) -> Result<()> {
        ...
        let params = InitializeParams {
            ...
            initialization_options: Some(self.get_initialization_options()),
            client_info: Some(ClientInfo {
                name: "lsif-indexer".to_string(),
                version: Some("0.1.0".to_string()),
            }),
    ...
    fn get_initialization_options(&self) -> serde_json::Value {
        use serde_json::json;
    ...
    pub fn get_document_symbols(&mut self, file_uri: &str) -> Result<Vec<DocumentSymbol>> {
        ...
        // 2-stage timeout configuration: 15 seconds for workspace/symbol operations
        let timeout = match self.language_id.as_str() {
            "nix" => Duration::from_secs(15), // 15 seconds for nixd workspace/symbol operations
            _ => self
                .health_checker
    ...
    pub fn search_workspace_symbols(&mut self, query: &str) -> Result<Vec<SymbolInformation>> {
        ...
        let timeout = match self.language_id.as_str() {
            "nix" => Duration::from_secs(15), // 15 seconds for nixd workspace/symbol operations
            _ => Duration::from_secs(30) // Default timeout for other languages
        };

        self.send_request_with_timeout::<_, Option<Vec<SymbolInformation>>>("workspace/symbol", params, timeout)?
...
pub fn detect_language(file_path: &str) -> Option<Box<dyn LspAdapter>> {
    ...
    match extension {
        ...
        "nix" => {
            // Import NixdAdapter when needed
            use super::nixd::NixdAdapter;
            Some(Box::new(NixdAdapter::new()))
        },
        _ => None,
    }
}
