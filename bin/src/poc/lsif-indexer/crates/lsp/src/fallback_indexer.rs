...
// Nix用の正規表現を静的に初期化
static NIX_LET_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(\w+)\s*=").unwrap());
static NIX_ATTR_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(\w+)\s*=\s*\{").unwrap());
static NIX_FUNCTION_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(\w+)\s*=\s*\w+\s*:").unwrap());
static NIX_IMPORT_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"^\s*(\w+)\s*=\s*import\s+").unwrap());
...
impl FallbackIndexer {
    ...
    pub fn extract_symbols(&self, file_path: &Path) -> Result<Vec<DocumentSymbol>> {
        ...
        match self.language {
            ...
            FallbackLanguage::Nix => self.extract_nix_symbols(&lines),
        }
    }
    ...
    /// Nixのシンボルを抽出
    fn extract_nix_symbols(&self, lines: &[&str]) -> Result<Vec<DocumentSymbol>> {
        let mut symbols = Vec::new();
        for (line_no, line) in lines.iter().enumerate() {
            ...
            if let Some(caps) = NIX_IMPORT_REGEX.captures(line) {
                let name = caps.get(1).unwrap().as_str().to_string();
                symbols.push(self.create_symbol(
                    name,
            ...
            else if let Some(caps) = NIX_ATTR_REGEX.captures(line) {
                let name = caps.get(1).unwrap().as_str().to_string();
                symbols.push(self.create_symbol(
                    name,
                    SymbolKind::OBJECT,
            ...
            else if let Some(caps) = NIX_FUNCTION_REGEX.captures(line) {
                let name = caps.get(1).unwrap().as_str().to_string();
                symbols.push(self.create_symbol(
                    name,
                    SymbolKind::FUNCTION,
            ...
            else if let Some(caps) = NIX_LET_REGEX.captures(line) {
                let name = caps.get(1).unwrap().as_str().to_string();
                symbols.push(self.create_symbol(
                    name,
                    SymbolKind::VARIABLE,
...
mod tests {
    ...
    fn test_fallback_language_from_extension() {
        ...
        // Nix
        assert!(FallbackIndexer::from_extension(Path::new("test.nix")).is_some());
...
let
  ...
  myImport = import ./other.nix;
  
in myPackage
"#;

        fs::write(&file_path, nix_code).unwrap();

        let indexer = FallbackIndexer::from_extension(&file_path).unwrap();
        let symbols = indexer.extract_symbols(&file_path).unwrap();

        // インポート、属性セット、関数、変数が検出されることを確認
        assert!(symbols
            .iter()
            .any(|s| s.name == "myImport" && s.kind == SymbolKind::NAMESPACE));
        assert!(symbols
            .iter()
            .any(|s| s.name == "myPackage" && s.kind == SymbolKind::OBJECT));
        assert!(symbols
            .iter()
            .any(|s| s.name == "myFunc" && s.kind == SymbolKind::FUNCTION));
        assert!(symbols
            .iter()
            .any(|s| s.name == "buildInputs" && s.kind == SymbolKind::VARIABLE));
        assert!(symbols
            .iter()
            .any(|s| s.name == "mySet" && s.kind == SymbolKind::OBJECT));
    }
}
