        ...
        configs.insert(
            ...
            LspServerConfig {
                ...
                initialization_options: Some(serde_json::json!({
                    ...
                    "options": {
                        "nixos": {
                            "expr": "import <nixpkgs/nixos> { configuration = import ./configuration.nix; }"
                        },
                        "home-manager": {
                            "expr": "import <home-manager/modules> { configuration = import ./home.nix; pkgs = import <nixpkgs> {}; }"
                        }
                    }
                })),
                workspace_folders: vec![],
...
mod tests {
    ...
    fn test_language_detection() {
        ...
        assert_eq!(
            LspServerRegistry::detect_language(Path::new("test.nix")),
            Some("nix".to_string())
        );
    }
    ...
    fn test_registry_default_configs() {
        ...
        assert!(registry.get_config("nix").is_some());
        ...
        let nix_config = registry.get_config("nix").unwrap();
        assert_eq!(nix_config.command, "nixd");
        assert_eq!(nix_config.language_id, "nix");
        assert_eq!(nix_config.args, vec!["--stdio"]);
        assert!(nix_config.initialization_options.is_some());
    }
