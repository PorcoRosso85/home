...
impl DifferentialIndexer {
    ...
    fn update_metadata(&mut self) -> Result<()> {
            ...
            .filter(|e| e.file_type().is_file())
        {
            ...
            if path
                ...
                .and_then(|s| s.to_str())
                .map(|ext| {
                    ext == "rs" || ext == "ts" || ext == "js" || ext == "tsx" || ext == "jsx" || ext == "nix"
                })
                .unwrap_or(false)
            {
    ...
    fn scan_all_files(&self) -> Result<Vec<FileChange>> {
        ...
        for entry_result in walkdir.into_iter() {
            ...
            if matches!(ext, "rs" | "ts" | "tsx" | "js" | "jsx" | "nix") {
    ...
    fn warm_up_lsp_clients(&mut self) -> Result<()> {
        ...
        // 環境変数で有効な言語が指定されているかチェック
        let enabled_languages = if let Ok(env_languages) = std::env::var("LSIF_ENABLED_LANGUAGES") {
            let enabled_set: HashSet<&str> = env_languages
                .split(',')
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .collect();
            ...
            if !enabled_set.is_empty() {
                info!("Environment variable LSIF_ENABLED_LANGUAGES detected: {:?}", enabled_set);
                Some(enabled_set)
            } else {
                None
            }
            ...
            .filter(|e| e.file_type().is_file())
        {
            ...
            if let Some(ext) = path.extension().and_then(|s| s.to_str()) {
                ...
                if let Some(l) = lang {
                    // 環境変数で制限されている場合は、その言語のみを使用
                    if let Some(ref enabled) = enabled_languages {
                        if enabled.contains(l) {
                            languages.insert(l);
                        } else {
                            debug!("Skipping language {} (not in LSIF_ENABLED_LANGUAGES)", l);
                        }
                    } else {
                        languages.insert(l);
                    }
        ...
        // 環境変数で指定された言語がプロジェクトに存在しない場合も強制的に含める
        if let Some(ref enabled) = enabled_languages {
            for &lang in enabled {
        ...
        if enabled_languages.is_some() {
            info!("Warming up LSP clients for ENABLED languages only: {:?}", langs);
        } else {
            info!("Warming up LSP clients for detected languages: {:?}", langs);
        }

        let warm_up_start = Instant::now();
        self.lsp_pool.warm_up(&self.project_root, &langs)?;
        info!(
            "LSP warm-up completed in {:.2}s for {} language(s)",
