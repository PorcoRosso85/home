...
ℹ️ — There is no need to set `PLAYWRIGHT_SKIP_VALIDATE_HOST_REQUIREMENTS = true` in this case, since there are no dynamic dependencies to check (because the chromium binary we specified in `executablePath` is a static executable).
