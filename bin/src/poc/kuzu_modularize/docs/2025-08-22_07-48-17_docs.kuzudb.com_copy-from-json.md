...
```
CREATE NODE TABLE Person (    p_id STRING,    name STRING,    info STRUCT(        height FLOAT,        weight FLOAT,        age UINT8,        insurance_provider STRUCT(type STRING, name STRING, policy_number STRING)[]    ),    PRIMARY KEY(p_id));COPY Person FROM 'people.json';
```
...
condition.json ```
[    {        "c_id": "c1",        "name": "Diabetes"    },    {        "c_id": "c2",        "name": "Hypertension"    }]
```

has\_condition.json ```
[    {        "from": "p1",        "to": "c1",        "since": 2019    },    {        "from": "p2",        "to": "c2",        "since": 2015    },    ...]
```
...
```
CREATE NODE TABLE Condition (    c_id STRING,    name STRING,    PRIMARY KEY(c_id));
```

Next, let’s create a relationship table `HAS_CONDITION`:

```
CREATE REL TABLE IF NOT EXISTS HAS_CONDITION(    FROM Patient TO Condition,    since UINT16)
```
