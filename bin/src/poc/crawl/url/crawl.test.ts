...
Deno.test.ignore(
  "URLCrawler - 同一ホスト制限（実際のネットワークアクセス）",
  async (t) => {
...
Deno.test.ignore(
  "URLCrawler - パターンマッチング（実際のネットワークアクセス）",
  async (t) => {
...
Deno.test.ignore(
  "URLCrawler - エラーハンドリング（実際のネットワークアクセス）",
  async (t) => {
    ...
    await t.step("タイムアウトをサポートする（将来実装）", () => {
      // TODO: タイムアウト機能を実装後に有効化
    });
  },
);

Deno.test.ignore(
  "URLCrawler - 出力形式（実際のネットワークアクセス）",
  async (t) => {
