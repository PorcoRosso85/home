...
1. Then, set the `ANTHROPIC_API_KEY` environment variable. We recommend storing this key securely (e.g., using a Github [secret](https://docs.github.com/en/actions/security-for-github-actions/security-guides/using-secrets-in-github-actions))
...
1. Otherwise, we invoke the MCP tool you provided in `--permission-prompt-tool`
...
```
1. **Handle errors gracefully** - check exit codes and stderr:
...
```
1. **Use session management** for maintaining context in multi-turn conversations
1. **Consider timeouts** for long-running operations:

```bash
timeout 300 claude -p "$complex_prompt" || echo "Timed out after 5 minutes"
```
1. **Respect rate limits** when making multiple requests by adding delays between calls
