...
# In-source tests
def test_parse_cypher_statements_コメント除去_正しくパース():
...
def test_generate_drop_statement_各種CREATE_DROP生成():
...
def test_handle_duplicate_rel_tables_重複テーブル_リネーム():
...
def test_apply_schema_正常系_スキーマ適用():
...
def test_rollback_適用済みスキーマ_ロールバック成功():
