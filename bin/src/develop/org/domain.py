...
from variables import DEVELOPER_WINDOW_PREFIX


# === Tool-specific Business Rules ===

def generate_developer_window_name(directory: str) -> str:
    """Generate tmux window name for Developer worker.
    
    Policy: Developer windows use 'developer:' prefix with underscored path.
    
    Args:
        directory: Directory path for the worker
        
    Returns:
        Window name following Developer naming convention
    """
    return f"{DEVELOPER_WINDOW_PREFIX}{directory.replace('/', '_')}"
...
def extract_directory_from_window_name(window_name: str, tool_prefix: str = None) -> str:
    ...
    Args:
        ...
        tool_prefix: Tool-specific prefix (default: "developer:")
        
    Returns:
        Original directory path
    """
    if tool_prefix is None:
        tool_prefix = DEVELOPER_WINDOW_PREFIX
...
def is_developer_window(window_name: str, tool_prefix: str = None) -> bool:
    """Check if a window name represents a developer.
