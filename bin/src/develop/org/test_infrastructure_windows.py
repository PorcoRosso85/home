...
class TestWindowManagement:
    ...
    def test_create_worker_window_creates_window_with_correct_naming(self, mock_server):
        ...
        # Verify window name follows developer: prefix with directory conversion
        expected_name = "developer:_home_user_project"
    ...
    def test_create_worker_window_handles_complex_directory_paths(self, mock_server):
        ...
        expected_name = "developer:_home_user_projects_my-app_src"
        mock_session.new_window.assert_called_once_with(window_name=expected_name)
    ...
    def test_list_all_worker_windows_filters_correctly(self, mock_server):
        """Test that list_all_worker_windows filters windows with developer: prefix."""
        # Create mock windows - some with developer: prefix, some without
        developer_window1 = Mock()
        developer_window1.name = "developer:_home_user_project1"
        developer_window2 = Mock()
        developer_window2.name = "developer:_home_user_project2"
        regular_window = Mock()
        ...
        mock_session = Mock()
        mock_session.windows = [developer_window1, regular_window, developer_window2, other_window]
        mock_server_instance = Mock()
        mock_server_instance.find_where.return_value = mock_session
        mock_server.return_value = mock_server_instance
        ...
        # Should only return windows with developer: prefix
        assert len(result) == 2
        assert developer_window1 in result
        assert developer_window2 in result
        assert regular_window not in result
        assert other_window not in result

    @patch('infrastructure.libtmux.Server')
    def test_list_all_worker_windows_returns_empty_when_no_developer_windows(self, mock_server):
        """Test that list_all_worker_windows returns empty list when no developer windows exist."""
        regular_window = Mock()
        regular_window.name = "main"
        other_window = Mock()
        other_window.name = "vim"
    ...
    def test_launch_claude_in_window_sends_correct_commands(self):
        ...
        # Verify commands were sent in correct order - using the actual claude-shell.sh script
        expected_calls = [
            (f"cd {directory}",),
